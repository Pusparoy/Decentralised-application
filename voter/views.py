    ...
    if (request.method == 'POST'):
        ...
        if (User.objects.filter(username=voterid)).exists():
            ...
            if Voters.objects.filter(voterid_no=voterid):
                ...
                url = "http://2factor.in/API/V1/ed9f0210-dccb-11ed-addf-0200cd936042/SMS/" + user_phone + "/AUTOGEN"
...
def otp(request):
    if (request.method == "POST"):
        ...
        url = "http://2factor.in/API/V1/ed9f0210-dccb-11ed-addf-0200cd936042/SMS/VERIFY/" + request.session[
    ...
    if (request.method == 'POST'):
        ...
        def getImagesAndLabels(path):
            ...
            for imagePath in imagePaths:
                ...
                faces = faceDetect.detectMultiScale(img_arr)
                for (x,y,w,h) in faces:
                    facess.append(img_arr[y:y+h,x:x+w])
                    Ids.append(id)
            return facess,Ids
        faces,ids = getImagesAndLabels(BASE_DIR + "\\TrainingImage\\")
...
def vote(request):
    if request.method == "POST":
        ...
        if vote.v.has_voted == 'no':
            ...
            rec.read(BASE_DIR+"\\TrainingImageLabel\\Trainer.yml")
            faceDetect = cv2.CascadeClassifier(BASE_DIR+"\\haarcascade_frontalface_default.xml")
            cam = cv2.VideoCapture(folder+"\\"+vidfilename)
            ...
            if flag==1:
                ...
                url = "http://2factor.in/API/V1/ed9f0210-dccb-11ed-addf-0200cd936042/SMS/" + vmobno + "/AUTOGEN"
                response = requests.request("GET", url)
                data = response.json()
                request.session['otp_session_data'] = data['Details']
                response_data = {'Message': 'Success'}
...
def subvoteotp(request):
    if (request.method == "POST"):
        userotp = request.POST['otp']
        url = "http://2factor.in/API/V1/ed9f0210-dccb-11ed-addf-0200cd936042/SMS/VERIFY/" + request.session['otp_session_data'] + "/" + userotp
        response = requests.request("GET", url)
        data = response.json()
        if data['Status'] == "Success":
            v_id = request.session['v_id']
